...
function buildSites1($batch)
{
    $ausgabe = "";     // JS
    $anzahl1 = 0;      // JS
    $anzahl2 = 0;      // JS (dat=anzahl2-anzahl1)
    $anzmin  = 6;      // JS
	...
	echo '<dir style="margin-left:11%; display:none;" id="sp1">';
        ...
        echo '<a href="#kombi">combined</a> &nbsp;<a href="#top">top</a>';
        echo ' &nbsp;<a href="#bot">bottom</a><br>';
        echo "</dir>\n";
...
// Hörbücher u. ä. - Aufruf per: mid3TagMp3AudioBookP1.php
...
// Hörbücher u. ä. - Aufruf per: mid3TagMp3AudioBookP1L.php
// -- P1L(!) :: Bild nur in erster MP3-Datei und Bild-Link in allen anderen
function buildSites2d($batch)
{
$GUI = "ID3-tags-powered-by:ID3 by GitHub.com/ttimer/GUI-for-mID3v2";
$LNK = "https://GitHub.com/ttimer/GUI-for-mID3v2/";
    ...
    foreach($batch as $key => $array) 
    {
        ...
        foreach ($array as $key2 => $array1) 
        {
        	...
        	if(!empty($_POST["picture"]) && isset($_POST["Xpicture"]))
        	  $Picture = $_POST["picture"];
        	if(!empty($_POST["apiclink"]) && isset($_POST["Xpicture"]))
        	...
        	if((isset($_POST["Xpicture"]) && $Picture != "") && 
        	  ...
        	  $Befehl .= " -p '$Picture'";
                ...
                if($Exe == "on") {
                  ...
                  if(isset($_POST["Xpicture"]) && $fcount == 0) {
                    if($PicLink != "" && $Picture != "") { 
                      echo "<dir style='text-align:center; font-size:1.2em; font-weight:bold;'><hr style='width:77%;'>";
                      echo "<hr style='width:77%;'>";
                      echo "<h3>APIC-Link (basic application)</h3>";
                      echo "<p>Updates are sent directly to all interested parties via email; </p>";
                      echo "<p>the dispatch takes place in the order of the requirements at: </p>";
                      echo "<p>J. Smolka - Oskamp 13 - 59394 Nordkirchen - Germany </p><br>";
                      echo "<p>P.S.: </p>";
                      echo "<p>Donations to support development are very welcome. </p>";
                      echo "<hr style='width:77%;'>";
                      echo "<hr style='width:77%;'>";
                      echo "<h3>APIC-Link (Basisprogramm)</h3>";
                      echo "<p>Aktualisierungen gehen per eMail direkt an alle Interessenten; </p>";
                      echo "<p>der Versand erfolgt in der Reihenfolge der Anforderungen bei: </p>";
                      echo "<p>J. Smolka - Oskamp 13 - 59394 Nordkirchen - Deutschland </p><br>";
                      echo "<p>P.S.: </p>";
                      echo "<p>Spenden zur Entwicklungsunterstuetzung sind willkommen. </p>";
                      echo "<hr style='width:77%;'>";
                      echo "<hr style='width:77%;'></dir>";
                    }
                  }
                  $fcount = $fcount + 1;
                }
...
//                     $tracknr = sprintf("%s",($tracknr + 1));
            ...
            // ALLE Tags LÖSCHEN
            if($Exe == "on" && $Artist == "jsDelete" && $Album == "jsDelete" && $Titel == "jsDelete") {
              echo("Delete All !");
              exec("mid3v2 -D '$Datei'");
            }
        }
